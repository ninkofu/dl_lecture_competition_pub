...
class Sequence(Dataset):
    def __init__(self, seq_path: Path, representation_type: RepresentationType, mode: str = 'test', delta_t_ms: int = 100,
        ...
        if self.mode == "train":
            if not flow_path.exists():
                raise FileNotFoundError(f"Directory not found: {flow_path}")
    ...
    def load_flow(flowfile: Path):
        ...
        return flow[:,:,:2], valid2D  # チャンネル数を2に制限
    ...
    def get_data(self, index) -> Dict[str, any]:
        ...
        if index + 1 < len(self.timestamps_flow):
            ts_start_next: int = self.timestamps_flow[index + 1] - self.delta_t_us
            ts_end_next: int = self.timestamps_flow[index + 1]
        else:
            ts_start_next: int = ts_start
            ts_end_next: int = ts_end
        ...
        if event_data is None or event_data_next is None:
            print(f"Index {index}: No event data found for the given timestamps.")
            return None
        ...
        print(f"Index {index}: Event volume shape {output['event_volume'].shape}")

        if self.load_gt:
            flow_file_path = self.flow_png[index]
            flow_gt_data = self.load_flow(flow_file_path)
            if flow_gt_data:
                output['flow_gt'] = [torch.tensor(x) for x in flow_gt_data]
                output['flow_gt'][0] = torch.moveaxis(output['flow_gt'][0], -1, 0)
                output['flow_gt'][1] = torch.unsqueeze(output['flow_gt'][1], 0)
                print(f"Index {index}: Flow GT shape {output['flow_gt'][0].shape}, File: {flow_file_path}")
            else:
                output['flow_gt'] = None
                print(f"Index {index}: No valid flow_gt found, File: {flow_file_path}")
    ...
    def __getitem__(self, index):
        ...
        if data is None:
            raise IndexError("Index out of range or data is None.")
        return data
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            sample['event_volume'] = self.transform(sample['event_volume'])
        return sample

class DatasetProvider:
    def __init__(self, dataset_path: Path, representation_type: RepresentationType, delta_t_ms: int, num_bins: int):
    ...
    def get_train_dataset(self):
        sequences = []
        for seq_path in sorted(self.dataset_path.glob('train/seq_*')):
        ...
        return torch.utils.data.ConcatDataset(sequences)

    def get_test_dataset(self):
        sequences = []
        for seq_path in sorted(self.dataset_path.glob('test/seq_*')):
            sequences.append(Sequence(
                seq_path=seq_path,
                representation_type=self.representation_type,
                mode='test',
...
def train_collate(batch):
    ...
    batch_dict['event_volume'] = torch.stack([b.clone().detach() for b in batch_dict['event_volume']], dim=0)
    
    if 'event_volume_next' in batch_dict:
        batch_dict['event_volume_next'] = torch.stack([b.clone().detach() for b in batch_dict['event_volume_next']], dim=0)
    
    if 'flow_gt' in batch_dict:
        if any(b is None for b in batch_dict['flow_gt']):
            print("Batch does not contain valid flow_gt")
            batch_dict['flow_gt'] = None
        else:
            valid_flow_gts = [torch.cat(b[:2], dim=0) if isinstance(b, list) and len(b) > 1 else b[:2] for b in batch_dict['flow_gt']]  # チャンネル数を2に制限
            if len(valid_flow_gts) > 0:
                batch_dict['flow_gt'] = torch.stack(valid_flow_gts, dim=0)
            else:
                print("No valid flow_gt tensors found")
                batch_dict['flow_gt'] = None
...
# class DatasetProvider:
#     def __init__(self, dataset_path, representation_type, delta_t_ms, num_bins):
#         self.dataset_path = dataset_path
#         self.representation_type = representation_type
#         self.delta_t_ms = delta_t_ms
#         self.num_bins = num_bins
...
#     def get_train_dataset(self):
#         # ここにデータ読み込みロジックを追加します
#         train_data = ... # データの読み込み
#         return CustomDataset(train_data, transform=transforms.Compose([
